...
simpleMinutes.initialize("user", (clock) => {
  ...
  if (clock.Date1 !== undefined) {
    // Position
    _dates1Container.x = (device.screen.width) - (clock.Date1.length * 20) - 26;
    // Values
  ...
  if (clock.Date2 !== undefined) {
    // Position
    _dates2Container.x = (device.screen.width) - (clock.Date2.length * 20) - 26;
    // Values
...
function UpdateActivities() {
  ...
  //activities.steps=new simpleActivities.Activity(8753,10000);
  // Steps
